 ...
 *
 * 2013-01-24: Ported from PHP4 to PHP5 by Mark Scherer - www.dereuromark.de
 * - PGR0/1 namespace now "SpreadsheetExcelReader", also the classname then
 * - Coding Standards
 * - PHP5 E_STRICT (partly)
 * - Cleanup and no non-class functions anymore (static OLERead ones to avoid conflicts)
 * - read() for files and readFromBlob() to directly process binary input
 * --------------------------------------------------------------------------
 */
...
class OLERead {
	...
	public function readFromBlob($data) {
		...
		$this->numBigBlockDepotBlocks = OLERead::getInt4d($this->data, NUM_BIG_BLOCK_DEPOT_BLOCKS_POS);
		$this->sbdStartBlock = OLERead::getInt4d($this->data, SMALL_BLOCK_DEPOT_BLOCK_POS);
		$this->rootStartBlock = OLERead::getInt4d($this->data, ROOT_START_BLOCK_POS);
		$this->extensionBlock = OLERead::getInt4d($this->data, EXTENSION_BLOCK_POS);
		$this->numExtensionBlocks = OLERead::getInt4d($this->data, NUM_EXTENSION_BLOCK_POS);
		...
		for ($i = 0; $i < $bbdBlocks; $i++) {
			$bigBlockDepotBlocks[$i] = OLERead::getInt4d($this->data, $pos);
			$pos += 4;
		}
		...
		for ($j = 0; $j < $this->numExtensionBlocks; $j++) {
			...
			if ($bbdBlocks < $this->numBigBlockDepotBlocks) {
				$this->extensionBlock = OLERead::getInt4d($this->data, $pos);
			}
		}
		...
		for ($i = 0; $i < $this->numBigBlockDepotBlocks; $i++) {
			...
			for ($j = 0 ; $j < BIG_BLOCK_SIZE / 4; $j++) {
				$this->bigBlockChain[$index] = OLERead::getInt4d($this->data, $pos);
				$pos += 4 ;
				$index++;
			}
		}
		...
		while ($sbdBlock != -2) {
			...
			for ($j = 0; $j < BIG_BLOCK_SIZE / 4; $j++) {
				$this->smallBlockChain[$index] = OLERead::getInt4d($this->data, $pos);
				$pos += 4;
				$index++;
			}
	...
	protected function _readPropertySets() {
		...
		while ($offset < strlen($this->entry)) {
			...
			$type = ord($d[TYPE_POS]);
			$startBlock = OLERead::getInt4d($d, START_BLOCK_POS);
			$size = OLERead::getInt4d($d, SIZE_POS);
			$name = '';
...
class SpreadsheetExcelReader {
	...
	public function dumpToArray($sheet = 0) {
		...
		for ($row = 0; $row < $this->rowcount($sheet); $row++) {
			for ($col = 0; $col < $this->colcount($sheet); $col++) {
				$arr[$row][$col] = iconv(null, 'utf-8', $this->val($row + 1, $col + 1, $sheet));
			}
		}
	...
	protected function _formatValue($format, $num, $f) {
		// 49==TEXT format
	 ...
	 */
	public function __construct($file = null, $store_extended_info = true, $outputEncoding = null) {
	...
	protected function _parse() {
		...
		$code = $this->_v($data, $pos);
		$length = $this->_v($data, $pos+2);
		$version = $this->_v($data, $pos+4);
		$substreamType = $this->_v($data, $pos+6);
			...
			switch ($code) {
				case SPREADSHEET_EXCEL_READER_TYPE_SST:
					...
					for ($i = 0; $i < $uniqueStrings; $i++) {
						...
						if ($spos == $limitpos) {
							$opcode = $this->_v($data, $spos);
							$conlength = $this->_v($data, $spos+2);
							if ($opcode != 0x3c) {
								return -1;
							}
							$spos += 4;
						...
						if ($richString) {
							// Read in the crun
							$formattingRuns = $this->_v($data, $spos);
							$spos += 2;
						}
				...
				case SPREADSHEET_EXCEL_READER_TYPE_FORMAT:
					$indexCode = $this->_v($data, $pos+4);
					if ($version == SPREADSHEET_EXCEL_READER_BIFF8) {
						$numchars = $this->_v($data, $pos+6);
						if (ord($data[$pos+8]) == 0) {
				...
				case SPREADSHEET_EXCEL_READER_TYPE_FONT:
						$height = $this->_v($data, $pos+4);
						$option = $this->_v($data, $pos+6);
						$color = $this->_v($data, $pos+8);
						$weight = $this->_v($data, $pos+10);
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
	...
	protected function _getCellDetails($spos, $numValue, $column) {
		...
		if ($type === 'date') {
			...
			$rectype = 'number';
			$formatted = $this->_formatValue($format, $numValue, $formatIndex);
	...
	protected function _GetIEEE754($rknum) {
		if (($rknum & 0x02) != 0) {
			...
			// 64 bit floating point $this->_value. The other 34 bits are assumed to be 0
