<div class="productSingles row">
	<div class="actions col-md-3 span3">
		<ul class="nav nav-list">
			<li class="nav-header"><?php echo __('Actions'); ?></li>
				...
				<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $this->Form->value('ProductSingle.id')), null, __('Are you sure you want to delete # %s?', $this->Form->value('ProductSingle.id'))); ?></li>
	...
	<?php echo $this->Form->create('ProductSingle', array('inputDefaults' => array( 'class' => 'form-control', 'div' => 'control-group', 'label' => array('class' => 'control-label'), 'wrapInput' => 'controls'), 'class' => 'form-horizontal'));?>
		<fieldset>
			<legend><?php echo __('Admin Edit Product Single'); ?></legend>
		<?php
		echo $this->Form->input('id');
		echo $this->Form->input('product_id', array('options'=>$products));
		echo $this->Form->input('title');
		echo $this->Form->input('preview', array('label'=>'Preview plaatje (http://placehold.it voor automatisch)'));
